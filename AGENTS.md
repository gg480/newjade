...
- **优先调度智能体**: SOLO Coder 收到任务后必须优先委派给对应智能体，禁止自行编写业务逻辑代码。发现自己在写代码应立即停止并重新调度。
- **Skill 驱动开发**: 有 Skill 规范覆盖的领域，开发团队必须严格按 Skill 规范执行。无 Skill 覆盖的任务，必须先走方案团队（研讨需求→搜寻案例→生成 Skill），再交给开发团队开发。
- **方案团队先行**: solution-consultant 负责需求研讨和方案评审，technical-researcher 负责搜寻外部案例。新需求领域若无 Skill 覆盖，禁止直接交给开发团队。
- **任务汇报写 Skill**: 各智能体完成任务后必须向 writing-expert 汇报关键流程，writing-expert 评估是否需要沉淀为 Skill（同类需求出现 2 次以上或流程复杂者必须创建/更新）。
